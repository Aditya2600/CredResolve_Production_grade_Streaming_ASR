...
async def transcribe(
    ...
    # Apply Audio Preprocessing (Plug-and-Play)
    vad_on = x_vad_enabled.lower() == "true"
    denoise_on = x_denoise_enabled.lower() == "true"
    ...
    if vad_on or denoise_on:
        try:
            ...
            if vad_on and not pcm:
                log.info("VAD filtered out all audio, returning empty transcript")
                return {
                    "text": "",
                    "language": x_language,
                    ...
                    "metrics": {
                        "audio_duration": 0.0,
                        "processing_latency": int((time.time() - t_pre_0)*1000)
                    }
                }
        except Exception as e:
            log.error("Audio preprocessing failed: %s", e)
            # Continue with original PCM if preprocessing fails to ensure resilience
