...
**Key design decision:** The gateway does **not** run noise suppression — only voice-activity gating. The historical `apm_enabled` flag has been removed from the public path; stale clients may still send it, but the gateway ignores it. Browser/device capture owns AEC, while worker-side preprocessing owns Silero VAD and denoise. Noise suppression lives exclusively in the worker, with **DeepFilterNet3** as the default backend and **RNNoise** retained as a one-release fallback (selectable via `DENOISER=rnnoise`).
...
[audio_deepfilternet_build.md](audio_deepfilternet_build.md). End-to-end image validation
