...
def load_voxtral_model_and_processor(device: torch.device):
    ...
    print(">>> Loading Voxtral Processor (mistralai/Voxtral-Mini-4B-Realtime-2602)...", flush=True)
    processor = VoxtralProcessor.from_pretrained("mistralai/Voxtral-Mini-4B-Realtime-2602", trust_remote_code=True)

    print(">>> Loading Voxtral Model weights (mistralai/Voxtral-Mini-4B-Realtime-2602)...", flush=True)
...
def transcribe_voxtral(model, processor, audio_arr, device: torch.device) -> str:
...
def main():
    parser = argparse.ArgumentParser(description="Test Voxtral-Mini-4B model on a single audio file or dataset sample.")
    ...
    print("="*80)
    print(" VOXTRAL-MINI-4B LOCAL DIAGNOSTIC & TEST UTILITY")
    print("="*80)
    ...
    try:
        model, processor = load_voxtral_model_and_processor(device)
    except Exception as e:
        print(f"Failed to load Voxtral Model/Processor: {e}")
        sys.exit(1)
    ...
    if args.all_samples:
        ...
        print("-"*80)
        print(f"Evaluating {len(samples)} samples with Voxtral locally...")
        ...
        for idx, sample in enumerate(samples):
            ...
            try:
                transcript = transcribe_voxtral(model, processor, audio_arr, device)
            except Exception as e:
                print(f"\nSample {idx} failed: {e}")
                continue
        ...
        pred_file = out_dir / f"predictions_voxtral_only_{timestamp}.jsonl"
        ...
        print("\n" + "="*80)
        print(" VOXTRAL-ONLY FULL BENCHMARK RESULTS")
        print("="*80)
        print(f"Model ID:          mistralai/Voxtral-Mini-4B-Realtime-2602")
    ...
    print("-"*80)
    print("Transcribing audio locally with Voxtral-Mini...")
    try:
        transcript = transcribe_voxtral(model, processor, audio_arr, device)
    except Exception as e:
